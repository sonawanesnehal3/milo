...
const setupKeyboardNav = async () => {
  keyboardNav = keyboardNav || new Promise(async (resolve) => {
    const KeyboardNavigation = await loadBlock('https://main--milo--adobecom.hlx.page/libs/blocks/global-navigation/utilities/keyboard/index.js');
    ...
    this.ready = this.ready || new Promise(async (resolve) => {
      try {
        ...
        const [
          ...
          loadBlock('https://main--milo--adobecom.hlx.page/libs/blocks/global-navigation/features/search/gnav-search.js'),
...
const cusnstomConfig = {
  ...
  //stage.adobe.com OR adobe.com
  // taxonomyRoot: '/your-path-here',
};

//=======================
export default async function init(block, consumerConfig) {

  console.log(block);
  debugger
  const nonMiloUrl = "https://main--milo--adobecom.hlx.page/drafts/snehal/fragments/my-gnav";
  try {
    setConfig({...cusnstomConfig, ...consumerConfig});
    const { locale, mep, mycfgvar} = getConfig();
    console.log(mycfgvar);
    const url = nonMiloUrl || getMetadata('gnav-source') || `${locale.contentRoot}/gnav`;
