...
var personalization = require('./personalization-b328591c.js');
require('./entitlements-3c8a94c6.js');

function updatePreviewButton() {
  const selectedInputs = document.querySelectorAll('.mep-popup input[type="radio"]:checked, .mep-popup input[type="text"]');
  ...
  const mepHighlightCheckbox = document.querySelector('.mep-popup input[type="checkbox"]#mepHighlightCheckbox');
  ...
  const mepPreviewButtonCheckbox = document.querySelector('.mep-popup input[type="checkbox"]#mepPreviewButtonCheckbox');
...
const decoratePreviewMode = function () {
  try {
    const {
      miloLibs,
      codeRoot,
    ...
    globalFooter.loadStyle(`${miloLibs || codeRoot}/features/personalization/preview.css`);
...
const getEditManifestUrl = function (a, w) {
  try {
    const repo = getRepo();
    if (!repo) {
    ...
    return Promise.resolve(fetch(`https://admin.hlx.page/status/adobecom/${repo}/main${a.dataset.manifestPath}?editUrl=auto`)).then(function (response) {
...
function createPreviewPill(manifests) {
  const overlay = globalFooter.createTag('div', {
    class: 'mep-preview-overlay static-links',
    style: 'display: none;'
  });
  ...
  manifests?.forEach(manifest => {
    ...
    const manifestFileName = personalization.getFileName(manifestPath);
    ...
    let analyticsTitle = '';
    if (manifestType === personalization.TRACKED_MANIFEST_TYPE) {
  ...
  const personalizationOn = globalFooter.getMetadata('personalization');
...
function addHighlightData(manifests) {
  manifests.forEach(({
    ...
    const manifestName = personalization.getFileName(manifest);
    ...
    selectedVariant?.replacefragment?.forEach(({
      val
    }) => updateManifestId(`[data-path*="${val}"]`));
    selectedVariant?.useblockcode?.forEach(({
      selector
    }) => {
      if (selector) updateManifestId(`.${selector}`, 'codeManifestId');
    });
    selectedVariant?.updatemetadata?.forEach(({
      selector
    }) => {
...
exports["default"] = decoratePreviewMode;
//# sourceMappingURL=preview-65955aa6.js.map
