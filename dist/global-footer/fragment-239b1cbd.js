...
const init = function (a) {
  try {
    ...
    let relHref = globalFooter.localizeLink(a.href);
    let inline = false;
    if (a.parentElement?.nodeName === 'P') {
      const children = a.parentElement.childNodes;
      const div = globalFooter.createTag('div');
      _iterator = globalFooter._createForOfIteratorHelper(a.parentElement.attributes);
      try {
    ...
    return Promise.resolve(Promise.resolve().then(function () { return require('./helpers-d88ce857.js'); })).then(function ({
      ...
      return Promise.resolve(customFetch({
        ...
        return Promise.resolve(resp.text()).then(function (html) {
          ...
          const fragment = globalFooter.createTag('div', {
            class: 'fragment',
            'data-path': relHref
          });
          if (!inline) {
          ...
          const _temp = function () {
            if (inline) {
              ...
              return Promise.resolve(globalFooter.loadArea(fragment)).then(function () {});
            }
          }();
...
const updateFragMap = (fragment, a, href) => {
  const fragLinks = [...fragment.querySelectorAll('a')].filter(link => globalFooter.localizeLink(link.href).includes('/fragments/'));
  if (!fragLinks.length) return;
  if (document.body.contains(a)) {
    // is fragment on page (not nested)
    // eslint-disable-next-line no-use-before-define
    fragMap[href] = new Tree(href);
    fragLinks.forEach(link => fragMap[href].insert(href, globalFooter.localizeLink(removeHash(link.href))));
  } else {
    Object.values(fragMap).forEach(tree => {
      if (tree.find(href)) {
        fragLinks.forEach(link => tree.insert(href, globalFooter.localizeLink(removeHash(link.href))));
      }
    });
  }
};
...
class Tree {
      ...
      return regeneratorRuntime.wrap(function _callee$(_context) {
        while (1) switch (_context.prev = _context.next) {
          ...
          case 2:
            ...
            _iterator2 = globalFooter._createForOfIteratorHelper(node.children);
            _context.prev = 4;
            _iterator2.s();
          case 6:
  ...
  insert(parentNodeKey, key, value = key) {
    var _iterator3 = globalFooter._createForOfIteratorHelper(this.traverse()),
      _step3;
    try {
  ...
  remove(key) {
    var _iterator4 = globalFooter._createForOfIteratorHelper(this.traverse()),
      _step4;
    try {
  ...
  find(key) {
    var _iterator5 = globalFooter._createForOfIteratorHelper(this.traverse()),
      _step5;
    try {
...
//# sourceMappingURL=fragment-239b1cbd.js.map
