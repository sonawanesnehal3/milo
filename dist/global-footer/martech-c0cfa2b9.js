...
const init = function ({
  ...
  try {
    ...
    const _temp2 = function () {
      if (persEnabled) {
        globalFooter.loadLink(`${config.miloLibs || config.codeRoot}/features/personalization/personalization.js`, {
          as: 'script',
          rel: 'modulepreload'
        });
...
const setDeep = (obj, path, value) => {
  ...
  let currentObj = obj;
  var _iterator = globalFooter._createForOfIteratorHelper(pathArr.slice(0, -1)),
    _step;
  try {
...
const getTargetPersonalization = function () {
  try {
    ...
    const timeout = parseInt(params.get('target-timeout'), 10) || parseInt(globalFooter.getMetadata('target-timeout'), 10) || TARGET_TIMEOUT_MS;
...
const setupEntitlementCallback = () => {
  const setEntitlements = function (destinations) {
    try {
      return Promise.resolve(Promise.resolve().then(function () { return require('./entitlements-3c8a94c6.js'); })).then(function ({
  ...
  globalFooter.loadLink(`${miloLibs || codeRoot}/features/personalization/entitlements.js`, {
    as: 'script',
    rel: 'modulepreload'
  });
};
  ...
  try {
    ...
    filesLoadedPromise = function () {
      try {
        ...
        return Promise.resolve(globalFooter.loadScript(`${config.miloLibs || config.codeRoot}/deps/${martechPath}`)).then(function () {
...
//# sourceMappingURL=martech-c0cfa2b9.js.map
