...
var entitlements = require('./entitlements-3c8a94c6.js');
...
const categorizeActions = function (experiment) {
  try {
    function _temp5() {
      selectedVariant.insertscript?.map(script => globalFooter.loadScript(script.val));
...
const getPersConfig = function (info, override = false) {
  try {
    function _temp3() {
      ...
      const globalConfig = globalFooter.getConfig();
      ...
      return Promise.resolve(getPersonalizationVariant(config.manifestPath, config.variantNames, variantLabel)).then(function (selectedVariantName) {
        ...
        if (placeholders) {
          globalFooter.updateConfig(parsePlaceholders(placeholders, globalFooter.getConfig(), config.selectedVariantName));
        }
        config.name = name;
...
const getPersonalizationVariant = function (manifestPath, variantNames = [], variantLabel = null) {
  try {
    ...
    return Promise.resolve(entitlements.getEntitlementMap()).then(function (_getEntitlementMap) {
      function _temp11() {
        const hasMatch = name => {
          if (name === '') return true;
...
const preloadManifests = ({
  ...
  var _iterator = globalFooter._createForOfIteratorHelper(manifests),
    _step;
  try {
    for (_iterator.s(); !(_step = _iterator.n()).done;) {
      const manifest = _step.value;
      if (!manifest.manifestData && manifest.manifestPath && !manifest.disabled) {
        globalFooter.loadLink(manifest.manifestPath, {
          as: 'fetch',
...
const createFrag = (el, url, manifestId) => {
  ...
  let frag = globalFooter.createTag('p', undefined, a);
  const isSection = el.parentElement.nodeName === 'MAIN';
  if (isSection) {
    frag = globalFooter.createTag('div', undefined, frag);
  }
  globalFooter.loadLink(`${globalFooter.localizeLink(a.href)}.plain.html`, {
    as: 'fetch',
    crossorigin: 'anonymous',
    rel: 'preload'
  });
...
exports.PERSONALIZATION_TAGS = PERSONALIZATION_TAGS;
exports.TRACKED_MANIFEST_TYPE = TRACKED_MANIFEST_TYPE;
exports.appendJsonExt = appendJsonExt;
exports.applyPers = applyPers;
exports.categorizeActions = categorizeActions;
exports.cleanAndSortManifestList = cleanAndSortManifestList;
exports.deleteMarkedEls = deleteMarkedEls;
exports.getFileName = getFileName;
exports.getPersConfig = getPersConfig;
exports.handleCommands = handleCommands;
exports.handleFragmentCommand = handleFragmentCommand;
exports.matchGlob = matchGlob;
exports.normalizePath = normalizePath;
exports.parseConfig = parseConfig;
exports.preloadManifests = preloadManifests;
exports.replaceInner = replaceInner;
//# sourceMappingURL=personalization-b328591c.js.map
