...
const controlJapaneseLineBreaks = function (config, scopeArea = document) {
  try {
    const disabled = globalFooter.getMetadata('jpwordwrap:disabled') === 'true' || false;
    const budouxThres = Number(globalFooter.getMetadata('jpwordwrap:budoux-thres')) || 2000;
    const budouxExcludeSelector = globalFooter.getMetadata('jpwordwrap:budoux-exclude-selector');
    const bwEnabled = globalFooter.getMetadata('jpwordwrap:bw-enabled') === 'true' || false;
    const bwExcludeSelector = globalFooter.getMetadata('jpwordwrap:bw-exclude-selector') || 'p';
    const lineBreakOkPatterns = (globalFooter.getMetadata('jpwordwrap:line-break-ok') || '').split(',');
    const lineBreakNgPatterns = (globalFooter.getMetadata('jpwordwrap:line-break-ng') || '').split(',');
...
const applyJapaneseLineBreaks = function (config, options = {}) {
  try {
    ...
    return Promise.resolve((function (t) { return Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespace(require(t)); }); })(`${base}/deps/budoux-index-ja.min.js`)).then(function ({
      ...
      const _temp = function () {
        if (bwEnabled) {
          return Promise.resolve((function (t) { return Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespace(require(t)); }); })(`${base}/deps/bw2.min.js`)).then(function (_import) {
...
exports.applyJapaneseLineBreaks = applyJapaneseLineBreaks;
exports["default"] = controlJapaneseLineBreaks;
exports.isBalancedWordWrapApplied = isBalancedWordWrapApplied;
exports.isWordWrapApplied = isWordWrapApplied;
//# sourceMappingURL=japanese-word-wrap-c7a5ec37.js.map
