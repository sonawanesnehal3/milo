 ...
 */
const interlink = function (path, language, limit = 1000) {
  try {
    const isExceptionLanguage = ['zh', 'ko', 'ja', 'th', 'he'].includes(language);
    const articleBody = document.querySelector('main');
    return Promise.resolve(fetch(`${path}?limit=${limit}`)).then(function (resp) {
      ...
      return Promise.resolve(resp.json()).then(function (json) {
        ...
        // scan article to filter keywords down to relevant ones
          ...
          Keyword
        }) => articleText.indexOf(Keyword.toLowerCase()) !== -1)
        ...
        articleBody.querySelectorAll('div > p:not([class])').forEach(p => {
          ...
          if (isExceptionLanguage || maxParaLinks > 0) {
            ...
            .filter(node => node.nodeType === Node.TEXT_NODE).forEach(textNode => {
              const matches = [];
...
exports.checkAndAddMatch = checkAndAddMatch;
exports["default"] = interlink;
//# sourceMappingURL=interlinks-934a4b00.js.map
