import { g as getConfig } from './global-footer-92adaea3.js';
...
const getEntitlementMap = function () {
  try {
    const {
      env,
      consumerEntitlements
    } = getConfig();
    if (env?.name === 'prod') return Promise.resolve({
      ...consumerEntitlements,
      ...ENTITLEMENT_MAP
    });
    return Promise.resolve(import('./stage-entitlements-1fd80cbc.js')).then(function ({
...
const getEntitlements = function (data) {
  return Promise.resolve(getEntitlementMap()).then(function (entitlementMap) {
...
export { init as default, getEntitlementMap };
//# sourceMappingURL=entitlements-8d8707ce.js.map
